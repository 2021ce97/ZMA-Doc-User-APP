...
import { Bot, User, Send, ChevronRight, Activity } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 pb-safe">
      ...
      <div className="bg-white flex items-center p-4 shadow-[0_2px_10px_rgba(0,0,0,0.02)] z-20 sticky top-0">
        {onBack && (
          <button onClick={onBack} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 ms-2">
            <ChevronRight size={24} className="text-slate-700" />
          </button>
        )}
        <div className="w-10 h-10 bg-sky-100 rounded-full flex items-center justify-center me-2">
           <Bot size={20} className="text-sky-600" />
        </div>
        <div className="me-3">
          <h1 className="font-bold text-slate-900">هوش مصنوعی طبی</h1>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 relative">
         <div className="absolute inset-0 bg-sky-50 opacity-30 pattern-grid pointer-events-none"></div>
         ...
         {messages.map((msg) => (
           <div key={msg.id} className={`flex max-w-[85%] relative z-10 ${msg.sender === 'user' ? 'self-end flex-row-reverse' : 'self-start'}`}>
             <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.sender === 'user' ? 'bg-slate-200 ms-2' : 'bg-sky-600 me-2'}`}>
             ...
             <div className={`p-3 rounded-2xl text-sm leading-relaxed shadow-sm ${
               ...
               : 'bg-sky-600 text-white rounded-ss-sm'
             }`}>
               {msg.text}
             </div>
           </div>
      ...
      <div className="bg-white p-3 border-t border-slate-200 flex gap-2 items-end z-20 pb-safe">
        <textarea 
          placeholder="علایم خود را بنویسید..."
          className="flex-1 bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent resize-none h-12 min-h-[48px] max-h-[120px]"
          value={input}
        ...
        <button 
          onClick={handleSend}
          disabled={!input.trim()}
          className="w-12 h-12 bg-sky-600 text-white rounded-xl flex items-center justify-center shrink-0 shadow-sm disabled:opacity-50 disabled:bg-slate-300 transition-colors"
        >
          <Send size={20} className="me-1" />
        </button>
      </div>
    </div>
  );
