...
import { ChevronRight, ChevronLeft, CalendarClock, CreditCard, ArrowLeft, CheckCircle2, WifiOff } from 'lucide-react';
...
export function BookingScreen({ doctorId, onBack, onHome }: BookingScreenProps) {
  ...
  const handleBooking = () => {
    if (!navigator.onLine) {
       ...
       localStorage.setItem('offline_booking', JSON.stringify({ doctorId, slot: selectedSlot, date: new Date().toISOString() }));
  ...
  if (step === 2) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 text-center bg-slate-50 dark:bg-slate-900 transition-colors">
        <div className={`w-24 h-24 ${offlineQueued ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-500' : 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-500 dark:text-emerald-400'} border border-transparent rounded-full flex items-center justify-center mb-6`}>
          {offlineQueued ? <WifiOff size={48} /> : <CheckCircle2 size={48} />}
        </div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
          {offlineQueued 
             ? (lang === 'en' ? 'Booking Queued' : 'درخواست در انتظار شبکه') 
             : (lang === 'en' ? 'Booking Confirmed!' : 'نوبت شما تایید شد!')}
        </h2>
        <p className="text-slate-500 dark:text-slate-400 mb-8 max-w-[280px]">
          {offlineQueued 
            ? (lang === 'en' ? 'Your booking is queued. It will be submitted automatically when you reconnect.' : 'درخواست شما ثبت شد و به محض اتصال مجدد به اینترنت به صورت خودکار نهایی خواهد شد.')
            : (lang === 'en' ? 'A confirmation SMS with your token number has been sent. Please arrive 30 minutes early.' : 'پیامک (SMS) تایید حاوی شماره توکن برای شما ارسال گردید. لطفا نیم ساعت قبل در کلینیک حاضر باشید.')}
        </p>
        ...
        <div className="bg-white dark:bg-slate-800 w-full rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-700 mb-8 max-w-sm text-start transition-colors">
          <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700 pb-3 mb-3">
             <span className="text-slate-500 dark:text-slate-400 text-sm">{lang === 'en' ? 'Token Number:' : 'شماره توکن:'}</span>
             <span className="font-bold text-xl text-sky-600 dark:text-sky-400">۴۵</span>
          </div>
          <div className="flex justify-between items-center pb-2">
             <span className="text-slate-500 dark:text-slate-400 text-sm">{lang === 'en' ? 'Doctor:' : 'داکتر:'}</span>
             <span className="font-semibold text-slate-900 dark:text-slate-100">{doctor.name}</span>
          </div>
          <div className="flex justify-between items-center pb-2">
             <span className="text-slate-500 dark:text-slate-400 text-sm">{lang === 'en' ? 'Time:' : 'زمان:'}</span>
             <span className="font-semibold text-slate-900 dark:text-slate-100">{lang === 'en' ? `Today, ${selectedSlot}` : `امروز، ${selectedSlot}`}</span>
          </div>
          <div className="flex justify-between items-center">
             <span className="text-slate-500 dark:text-slate-400 text-sm">{lang === 'en' ? 'Payment:' : 'پرداخت:'}</span>
             <span className="font-semibold text-emerald-600 dark:text-emerald-400">{lang === 'en' ? `Cash (${doctor.fees} AFN)` : `نقدی (${doctor.fees} AFN)`}</span>
          </div>
        </div>
        )}

        <button 
          onClick={() => { triggerHaptic('light'); onHome(); }}
  ...
  return (
    <div className="flex flex-col pb-24 relative bg-slate-50 dark:bg-slate-900 min-h-full transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 flex items-center justify-between p-4 sticky top-0 z-20 shadow-sm border-b border-slate-200 dark:border-slate-700 transition-colors">
        ...
        <h1 className="font-bold text-slate-900 dark:text-white">{lang === 'en' ? 'Appointment Details' : 'مشخصات نوبت'}</h1>
      ...
      {bookingError && (
        <div className="mx-5 mt-4 p-3 bg-rose-50 dark:bg-rose-900/30 border border-rose-200 dark:border-rose-800 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4 z-10 transition-colors">
           ...
           <p className="text-xs text-rose-700 dark:text-rose-300 font-medium leading-relaxed">{bookingError}</p>
        </div>
      )}
      ...
      <div className="p-5">
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-4 mb-6 transition-colors">
          <img src={doctor.avatar} alt="" className="w-16 h-16 rounded-xl object-cover bg-slate-100 dark:bg-slate-700" loading="lazy" />
          <div>
            <h3 className="font-bold text-slate-900 dark:text-white">{doctor.name}</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{doctor.specialty}</p>
          </div>
        </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 snap-x hide-scrollbar mb-4">
          {dates.map((d, i) => (
            ...
            >
              <div className={`text-xs mb-1 ${i === 0 ? 'text-sky-100' : 'text-slate-500 dark:text-slate-400'}`}>{d.day}</div>
              <div className={`font-bold ${i === 0 ? 'text-white' : 'text-slate-900 dark:text-white'}`}>{d.date}</div>
            </button>
          ))}
        </div>

        <h3 className="font-bold text-slate-900 dark:text-white mb-3 mt-2">{lang === 'en' ? 'Select Time' : 'انتخاب ساعت'}</h3>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-sky-200 dark:border-sky-800 p-4 relative overflow-hidden ring-1 ring-sky-600 dark:ring-sky-700 shadow-sm transition-colors">
          ...
          <div className="font-bold text-slate-900 dark:text-white text-base mb-1.5">{lang === 'en' ? 'Cash Payment' : 'پرداخت نقدی (Cash)'}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed max-w-[85%]">
            {lang === 'en' ? 'Please pay the fee at the clinic.' : 'فیس نوبت را پس از مراجعه به معاینه خانه نقداً پرداخت نمایید.'}
          </div>
          <div className="mt-4 font-black flex items-center gap-2 text-emerald-600 dark:text-emerald-400 text-base">
            <span className="bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded border border-emerald-100 dark:border-emerald-800">{doctor.fees} AFN</span>
          </div>
        </div>
      </div>
