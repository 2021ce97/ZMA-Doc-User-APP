...
export function PharmacyScreen({ onBack }: PharmacyScreenProps) {
  ...
  const mapCategoryToPersian = (cat: string) => {
    switch (cat) {
      ...
      case "General Medicine":
        return "داروهای عمومی";
      case "Herbal":
        return "گیاهی";
      case "Cosmetics & Hygiene":
  ...
  const filteredPharmacies = PHARMACIES.filter((p) => {
    const matchesSearch =
      p.name.includes(searchQuery) || p.location.includes(searchQuery);
    const mappedSelected = selectedCategory
      ? mapCategoryToPersian(selectedCategory)
      : "همه";
    const matchesCategory =
      mappedSelected && mappedSelected !== "همه"
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 pb-safe transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 flex items-center justify-between p-4 sticky top-0 z-20 shadow-sm border-b border-slate-200 dark:border-slate-700 transition-colors">
        ...
        <h1 className="font-bold text-slate-900 dark:text-white">
          {lang === "en" ? "Pharmacy Search" : "جستجوی درملتون (Pharmacy)"}
        </h1>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 border-b border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
          <div className="relative mb-4">
            <input
              ...
              placeholder={
                lang === "en"
                  ? "Pharmacy name or area..."
                  : "نام درملتون یا منطقه..."
              }
        ...
        <div className="p-4 space-y-4">
          {filteredPharmacies.map((pharmacy) => (
            <div
              key={pharmacy.id}
              className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-4 shadow-sm flex flex-col gap-3 transition-colors"
            >
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                      ...
                      <span>
                        {pharmacy.distance
                          .replace("کیلومتر", lang === "en" ? "km" : "کیلومتر")
                          .replace(/[۰-۹]/g, (c) =>
                            lang === "en" ? String("۰۱۲۳۴۵۶۷۸۹".indexOf(c)) : c,
                          )}
                      </span>
                    </div>
                  </div>
                ...
                {pharmacy.isOpen ? (
                  <div className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-1 rounded">
                    {lang === "en" ? "Open" : "باز است"}
                  </div>
                ) : (
                  <div className="text-[10px] font-bold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/30 px-2 py-1 rounded">
                    {lang === "en" ? "Closed" : "بسته است"}
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {pharmacy.categories.map((cat, i) => {
                  const dict: Record<string, string> = {
                    "داروهای عمومی": "General Medicine",
                    "نسخه داکتر": "Prescription",
                    "لوازم بهداشتی": "Hygiene Supplies",
                    "داروهای تخصصی": "Specialized",
                    گیاهی: "Herbal",
                    "شیر خشک": "Baby Formula",
                    عمومی: "General",
                  ...
                  return (
                    <span
                      key={i}
                      className="text-[10px] bg-slate-50 dark:bg-slate-700 border border-slate-100 dark:border-slate-600 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-md transition-colors"
                    >
