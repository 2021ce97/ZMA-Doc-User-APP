...
import { ChevronRight, ChevronLeft, Heart, Share2, Search, BookOpen, Clock } from 'lucide-react';
...
export function BlogScreen() {
  ...
  const articles = [
    ...
    },
    {
      ...
      content: 'فشار خون بالا قاتل خاموش نامیده می‌شود زیرا در بسیاری از مواقع بدون علامت است...'
    }
  ];
      ...
      <div className="flex flex-col h-full bg-white pb-safe overflow-y-auto">
        <div className="relative">
           ...
           <div className="absolute top-0 inset-x-0 p-4 bg-gradient-to-b from-black/50 to-transparent flex justify-between">
              <button onClick={() => setSelectedArticleId(null)} className="w-10 h-10 flex items-center justify-center rounded-full bg-black/20 backdrop-blur-sm text-white hover:bg-black/30 transition-colors">
        ...
        <div className="p-5 -mt-6 bg-white rounded-t-3xl relative z-10">
          <span className="text-xs font-bold text-sky-700 bg-sky-50 px-3 py-1 rounded-full">{article.category}</span>
          <h1 className="font-bold text-slate-900 text-xl leading-relaxed mt-4 mb-3">{article.title}</h1>
          
          <div className="flex items-center gap-4 text-xs text-slate-500 mb-6 border-b border-slate-100 pb-4">
             <div className="flex items-center gap-1.5 font-medium"><Clock size={16} className="text-slate-400" /> {article.readTime} مطالعه</div>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 pb-safe">
      <div className="bg-white flex items-center justify-between p-4 sticky top-0 z-20 shadow-sm border-b border-slate-200">
        <h1 className="font-bold text-slate-900 text-lg mx-auto">{t('health_blog')}</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4">
          ...
          <h2 className="font-bold text-slate-800 mb-4">{t('latest_articles')}</h2>

          <div className="space-y-4">
            {articles.map(article => (
              <div key={article.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 flex flex-col cursor-pointer hover:shadow-md transition-shadow" onClick={() => setSelectedArticleId(article.id)}>
