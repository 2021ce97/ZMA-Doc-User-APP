...
export function SearchScreen({
  ...
  const [availFilter, setAvailFilter] = useState<"all" | "today" | "tomorrow">(
    "all",
  );
  ...
  const results = DOCTORS.filter((d) => {
    ...
    if (selectedFilter !== "همه" && selectedFilter !== "All") {
      matchesFilter = searchStr.includes(
        normalize(mapFilterToPersian(selectedFilter)),
      );
    }
    ...
    if (onlineBooking)
      advancedMatch = advancedMatch && d.onlineBooking === true;

    // Granular filters (using random assignment simulation for mock data if properties aren't there)
    if (insurance !== "all") {
      // Mock: doctor ids 1,3 accept insurance A etc. We'll simulate by checking id string
      advancedMatch =
        advancedMatch && parseInt(d.id) % 2 === (insurance === "Bima" ? 0 : 1);
    }
    if (language !== "all") {
      advancedMatch = advancedMatch && d.id !== "fallback-id"; // just a mock logic
    }
    if (procedure !== "all") {
      advancedMatch = advancedMatch && searchStr.includes(normalize(procedure));
    }
        ...
        <div className="bg-white dark:bg-slate-800 p-5 border-b border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <button
              ...
              className={`py-1.5 px-3 rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5 border ${
                useLocation
                  ? "bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 border-transparent"
                  : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300"
              }`}
            >
              <Navigation
                size={12}
                className={useLocation ? "fill-current" : ""}
              />
              {t("near_me")}
            </button>
            <div className="flex bg-slate-100 dark:bg-slate-700 p-0.5 rounded-lg border border-slate-200 dark:border-slate-600">
              <button
                onClick={() => {
                  triggerHaptic("light");
                  setAvailFilter(availFilter === "today" ? "all" : "today");
                }}
              ...
              <button
                onClick={() => {
                  triggerHaptic("light");
                  setAvailFilter(
                    availFilter === "tomorrow" ? "all" : "tomorrow",
                  );
                }}
                className={`py-1 px-3 rounded-md text-xs font-bold transition-colors flex items-center gap-1.5 ${
                  availFilter === "tomorrow"
                    ? "bg-sky-500 text-white shadow-sm"
                    : "text-slate-600 dark:text-slate-300"
                }`}
              >
            ...
            >
              ...
              {t("online_booking")}
              {onlineBooking && (
                <span className="absolute bottom-0 right-0 w-2 h-2 bg-indigo-500 rounded-tl-full"></span>
              )}
            </button>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
            <select
              ...
              className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs rounded-lg px-2 py-1.5 focus:ring-1 focus:ring-sky-500"
            >
              <option value="all">
                {lang === "en" ? "Any Insurance" : "همه بیمه‌ها"}
              </option>
              <option value="Bima">{lang === "en" ? "Bima" : "بیمه"}</option>
              <option value="Salam">{lang === "en" ? "Salam" : "سلام"}</option>
              <option value="Oqaab">{lang === "en" ? "Oqaab" : "عقاب"}</option>
            </select>
            <select
              value={language}
            ...
            >
              <option value="all">
                {lang === "en" ? "Any Language" : "همه زبان‌ها"}
              </option>
              <option value="Dari">{lang === "en" ? "Dari" : "دری"}</option>
              <option value="Pashto">
                {lang === "en" ? "Pashto" : "پښتو"}
              </option>
            ...
            >
              <option value="all">
                {lang === "en" ? "Any Procedure" : "همه عمل‌ها"}
              </option>
              <option value="Surgery">
                {lang === "en" ? "Surgery" : "جراحی"}
              </option>
              <option value="Ultrasound">
                {lang === "en" ? "Ultrasound" : "سونوگرافی"}
              </option>
            </select>
          </div>
        </div>
        ...
        <div className="p-4 space-y-3">
          {isLoading ? (
            Array.from({ length: 4 }).map((_, idx) => (
              <div
                key={idx}
                className="bg-white dark:bg-slate-800 w-full rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col relative animate-pulse transition-colors"
              >
