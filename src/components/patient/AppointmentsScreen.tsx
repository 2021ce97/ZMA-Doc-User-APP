...
export function AppointmentsScreen() {
    ...
    return [
      {
        ...
        phone: "۰۷۸۱۲۳۴۵۶۷",
        instructions:
          "لطفاً ۱۵ دقیقه قبل از زمان تعیین شده در شفاخانه حاضر باشید. سوابق پزشکی و نسخه قبلی خود را به همراه داشته باشید.",
      },
      {
        id: "2",
  ...
  const translateData = (val: string) => {
    ...
    const dict: Record<string, string> = {
      ...
      "لطفاً ۱۵ دقیقه قبل از زمان تعیین شده در شفاخانه حاضر باشید. سوابق پزشکی و نسخه قبلی خود را به همراه داشته باشید.":
        "Please arrive 15 minutes early. Bring your medical records.",
      ...
      "کارت واکسیناسیون طفل را حتما با خود بیاورید.":
    ...
    return (
      dict[val] ||
      val.replace(/[۰-۹]/g, (c) => "0123456789"["۰۱۲۳۴۵۶۷۸۹".indexOf(c)])
    );
  };
  ...
  const handleReschedule = (e: React.MouseEvent, id: string) => {
    ...
    if (
      !window.confirm(
        lang === "en"
          ...
          : "آیا مطمئن هستید که می‌خواهید این نوبت را تغییر دهید؟",
      )
    ) {
      return;
    }
    const newDate = window.prompt(
      lang === "en"
        ? "Enter new date (e.g., 2026-05-01):"
        : "تاریخ جدید را وارد کنید (مثال: ۲۰۲۶-۰۵-۰۱):",
    );
    if (newDate) {
      const newTime = window.prompt(
        lang === "en"
          ? "Enter new time (e.g., 10:00 AM):"
          : "زمان جدید را وارد کنید (مثال: ۱۰:۰۰ ق.ظ):",
      );
      if (newTime) {
        setAppointments((prev) =>
          prev.map((app) =>
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "upcoming":
        return "bg-sky-50 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400 border-sky-100 dark:border-sky-800";
      case "completed":
            ...
            {dateRange === "custom" && (
              <div className="mt-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl p-3 shadow-sm animate-in fade-in slide-in-from-top-2 transition-colors">
                <div className="flex items-center justify-between mb-3">
                  <button
                    onClick={prevMonth}
                    className="p-1 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-md transition-colors text-slate-700 dark:text-slate-300"
                  >
                    <ChevronRight
                      size={16}
        ...
        <div className="p-4 space-y-4">
          {filteredAppointments.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 px-6 text-center bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm relative overflow-hidden mt-4 transition-colors">
              ...
              <div
                className={`w-20 h-20 flex items-center justify-center rounded-3xl mb-4 relative z-10 animate-bounce shadow-sm transition-colors ${
                  selectedStatus === "cancelled"
                    ? "bg-rose-100 dark:bg-rose-900/40 border-4 border-white dark:border-slate-800"
                    : selectedStatus === "completed"
                      ? "bg-emerald-100 dark:bg-emerald-900/40 border-4 border-white dark:border-slate-800"
            ...
            filteredAppointments.map((app) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute top-4 ${isRtl ? "end-4" : "start-4"} text-[10px] font-bold px-2 py-1 rounded border transition-colors ${getStatusColor(app.status)}`}
                  >
                    {getStatusLabel(app.status)}
                  </div>

                  <div
                    className={`flex flex-col flex-wrap ${isRtl ? "pe-24" : "ps-24"}`}
                  >
                    <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                      {translateData(app.doctorName)}
                    </h3>
                    <p className="text-xs text-sky-600 dark:text-sky-400 font-medium">
                      {translateData(app.specialty)}
                    </p>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-50 dark:border-slate-700/50 space-y-2">
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                        <Calendar size={14} />{" "}
                        {t("date_filter").replace(":", "")}
                      </span>
                      <span
                        className="font-bold text-slate-700 dark:text-slate-300"
                        dir="ltr"
                      >
                        {translateData(app.date)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                        <Clock size={14} /> {lang === "en" ? "Time:" : "زمان:"}
                      </span>
                      <span className="font-bold text-slate-700 dark:text-slate-300">
                        {translateData(app.time)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500 flex items-center gap-1.5 dark:text-slate-400">
                        <X size={14} className="opacity-0" />{" "}
                        {lang === "en" ? "Location:" : "موقعیت:"}
                      </span>
                      <span className="font-medium text-slate-700 dark:text-slate-300 text-xs text-end">
                        {translateData(app.hospital)}
                      </span>
                    </div>
                  </div>
                  ...
                  {isExpanded && (
                    <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700/50 space-y-3 animate-in fade-in slide-in-from-top-2">
                      ...
                      {app.phone && (
                        <div className="text-sm">
                          <span className="text-slate-500 dark:text-slate-400 font-medium block mb-1">
                            {lang === "en" ? "Contact:" : "شماره تماس:"}
                          </span>
                          <span
                            className="text-slate-700 dark:text-slate-300 block"
                            dir="ltr"
                          >
                            {translateData(app.phone)}
                          </span>
                        </div>
                      )}
                      {app.instructions && (
                        <div className="text-sm bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg border border-amber-100 dark:border-amber-800/30">
                          <span className="text-amber-700 dark:text-amber-400 font-medium block mb-1">
                            {lang === "en"
                              ? "Pre-appointment Instructions:"
                              : "دستورالعمل‌های قبل از نوبت:"}
                          </span>
                          <span className="text-amber-600 dark:text-amber-300/80 block leading-relaxed text-xs">
                            {translateData(app.instructions)}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  {app.status === "upcoming" && (
                    <div className="mt-4 flex gap-2">
                      <button
                        onClick={(e) => handleReschedule(e, app.id)}
                        className="flex-1 bg-sky-50 hover:bg-sky-100 dark:bg-sky-900/30 dark:hover:bg-sky-900/50 text-sky-700 dark:text-sky-400 py-2.5 rounded-xl text-sm font-bold transition-colors active:scale-[0.98]"
                      >
                        {t("reschedule")}
                      </button>
                      <button
                        onClick={(e) => handleCancel(e, app.id)}
                        className="flex-1 bg-white hover:bg-rose-50 dark:bg-slate-800 dark:hover:bg-rose-900/20 border border-rose-200 dark:border-rose-900/50 text-rose-600 dark:text-rose-400 py-2.5 rounded-xl text-sm font-bold transition-colors active:scale-[0.98]"
                      >
