...
import { ChevronRight, Heart, Share2, MapPin, Award, Users, CreditCard, Calendar as CalendarIcon, Clock, Star, ShieldCheck, Building2, ChevronLeft } from 'lucide-react';
...
export function DoctorProfileScreen({ doctorId, onBack, onBook, isFavorite, onToggleFavorite }: DoctorProfileScreenProps) {
  ...
  const getAvailableDays = () => {
    ...
    if (timingStr.includes('همه') || timingStr.includes('تمام روزها') || timingStr.includes('هر روز')) {
  ...
  return (
    <div className="flex flex-col pb-24 relative bg-slate-50 min-h-full">
        ...
        <div className="flex gap-2">
          ...
          <button 
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-full transition-colors ${
              isFavorite ? 'bg-rose-50 text-rose-500 hover:bg-rose-100' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'
            }`}
          >
      ...
      <div className="bg-white p-5 shadow-sm">
        <div className="flex gap-4">
          <img src={doctor.avatar} alt={doctor.name} className="w-24 h-24 rounded-2xl object-cover bg-slate-100 shadow-sm border-2 border-slate-50" />
        ...
        <div className="flex justify-between mt-6 bg-slate-50 p-2 rounded-xl border border-slate-100">
          <div className="flex flex-col items-center flex-1 border-e border-slate-200 last:border-0 pe-2">
            ...
            <div className="font-bold text-slate-900 text-sm">{doctor.rating}</div>
            <div className="text-[9px] text-slate-500 tracking-tight uppercase">{t('ratings')}</div>
          </div>
          <div className="flex flex-col items-center flex-1 border-e border-slate-200 last:border-0 px-2">
            ...
            <div className="font-bold text-slate-900 text-sm">{doctor.patients}+</div>
            <div className="text-[9px] text-slate-500 tracking-tight uppercase">{t('patients')}</div>
          </div>
          <div className="flex flex-col items-center flex-1 ps-2">
            ...
            <div className="font-bold text-slate-900 text-sm">{doctor.experience} {t('years')}</div>
            <div className="text-[9px] text-slate-500 tracking-tight uppercase">{t('experience')}</div>
          </div>
        </div>
      </div>
      )}
      ...
      <div className="p-5 mt-2 bg-white border-y border-slate-100 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-1 rounded-md font-bold">{t('current_week')}</span>
        </div>
      ...
      <div className="p-5">
        <h3 className="font-bold text-slate-900 mb-2">{t('main_clinic_location')}</h3>
      ...
      <div className="fixed bottom-0 inset-x-0 p-4 bg-white border-t border-slate-200 z-30 flex gap-3 max-w-[400px] mx-auto pb-safe">
        <div className="flex flex-col justify-center px-4 bg-slate-50 rounded-xl border border-slate-100">
           <span className="text-[10px] text-slate-500 text-center">{t('next_appointment')}</span>
           <span className="text-xs font-bold text-sky-700 whitespace-nowrap pt-0.5" dir="ltr">{doctor.nextAvailable.split('،')[1]}</span>
        </div>
        <button 
