...
import { User, Settings, Heart, Bell, Shield, ChevronRight, LogOut, FileText, ChevronLeft, MapPin, Star, Clock, Globe, Moon, MessageSquare, X } from 'lucide-react';
...
export function ProfileScreen({ favorites, onNavigate, onToggleFavorite }: ProfileScreenProps) {
  ...
  const [notifSettings, setNotifSettings] = useState({ appointments: true, blog: false, ai: true });
  ...
  const submitFeedback = () => {
    ...
    alert(isRtl ? 'بازخورد شما با موفقیت ارسال شد!' : 'Feedback submitted successfully!');
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 pb-safe transition-colors">
      <div className="bg-sky-600 dark:bg-slate-800 pt-10 pb-20 px-5 relative overflow-hidden shrink-0 transition-colors">
         ...
         <h1 className="text-xl font-bold text-white text-center">{t('profile')}</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 -mt-14 relative z-10 hide-scrollbar pb-10">
         ...
         <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-4 mb-6 transition-colors">
            ...
            <div className="flex-1 text-start">
               <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">احمد جواد</h2>
               <p className="text-sm text-slate-500 dark:text-slate-400">۰۷۹ ۱۲۳ ۴۵۶۷</p>
            </div>
            ...
            {favoriteDoctors.length === 0 ? (
               <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 text-center shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center gap-4 relative overflow-hidden transition-colors">
                  ...
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-rose-100 to-sky-100 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center border-4 border-white dark:border-slate-800 shadow-sm z-10 animate-bounce">
                    <Heart size={28} className="text-rose-400 dark:text-rose-300 fill-rose-200 dark:fill-rose-400" />
                  </div>
                  <div className="z-10">
                    <h4 className="font-bold text-slate-800 dark:text-slate-200 text-base mb-1">{t('no_favorites')}</h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400 max-w-[220px] leading-relaxed mx-auto">
                      {t('favorites_desc')}
                    </p>
                  </div>
                  <button onClick={() => onNavigate('search')} className="mt-2 text-sm font-bold text-white bg-sky-600 hover:bg-sky-700 dark:bg-sky-500 dark:hover:bg-sky-600 transition-colors px-6 py-2.5 rounded-xl shadow-sm z-10 flex items-center gap-2">
                    {t('search_doctor')}
                  </button>
               </div>
            ) : (
                  ...
                  {favoriteDoctors.map(doctor => (
                     ...
                     >
                        ...
                        <div className="flex gap-3 mb-3">
                           ...
                           <div>
                           <h3 className="font-bold text-sm text-slate-900 dark:text-slate-100">{doctor.name}</h3>
         ...
         <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden mb-6 flex flex-col transition-colors">
            <MenuRow icon={<FileText />} color="text-blue-500" bg="bg-blue-50 dark:bg-blue-900/30" title={t('medical_records')} isRtl={isRtl} />
            ...
            <div className="relative border-b border-slate-50 dark:border-slate-700">
               ...
               >
                  <div className="flex items-center gap-3">
                     ...
                     <span className="font-medium text-slate-700 dark:text-slate-200 text-sm">{t('dark_mode')}</span>
                  </div>
            ...
            <div className="relative border-b border-slate-50 dark:border-slate-700">
               <button 
                 ...
                 onClick={() => { triggerHaptic('light'); setShowLangMenu(!showLangMenu); }}
               >
                  <div className="flex items-center gap-3">
                     <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500">
                        <Globe size={20} />
                     </div>
                     <span className="font-medium text-slate-700 dark:text-slate-200 text-sm">{t('language')}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-400 font-bold uppercase">{lang}</span>
                    {isRtl ? <ChevronLeft size={18} className="text-slate-300" /> : <ChevronRight size={18} className="text-slate-300" />}
                  </div>
               </button>
               {showLangMenu && (
                 <div className="bg-slate-50 dark:bg-slate-900/50 p-2 flex flex-col gap-1 border-t border-slate-100 dark:border-slate-700">
                   <button onClick={() => { triggerHaptic('light'); setLang('da'); setShowLangMenu(false); }} className={`p-2 text-sm text-start rounded-md ${lang === 'da' ? 'bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 font-bold' : 'text-slate-600 dark:text-slate-400'}`}>دری (Dari)</button>
                   <button onClick={() => { triggerHaptic('light'); setLang('ps'); setShowLangMenu(false); }} className={`p-2 text-sm text-start rounded-md ${lang === 'ps' ? 'bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 font-bold' : 'text-slate-600 dark:text-slate-400'}`}>پښتو (Pashto)</button>
                   <button onClick={() => { triggerHaptic('light'); setLang('en'); setShowLangMenu(false); }} className={`p-2 text-sm text-start rounded-md ${lang === 'en' ? 'bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 font-bold' : 'text-slate-600 dark:text-slate-400'}`}>English</button>
                 </div>
               )}
            </div>
            ...
            <div className="relative border-b border-slate-50 dark:border-slate-700">
               <button 
                 className="w-full flex items-center justify-between p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                 onClick={() => { triggerHaptic('light'); setShowNotifMenu(!showNotifMenu); }}
               >
                  <div className="flex items-center gap-3">
                     <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-amber-50 dark:bg-amber-900/30 text-amber-500">
                        <Bell size={20} />
                     </div>
                     <span className="font-medium text-slate-700 dark:text-slate-200 text-sm">{t('notifications')}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {isRtl ? (
                      <ChevronLeft size={18} className={`text-slate-300 transition-transform ${showNotifMenu ? '-rotate-90' : ''}`} />
                    ) : (
                      <ChevronRight size={18} className={`text-slate-300 transition-transform ${showNotifMenu ? 'rotate-90' : ''}`} />
                    )}
                  </div>
               </button>
               {showNotifMenu && (
                 <div className="bg-slate-50 dark:bg-slate-900/50 p-4 flex flex-col gap-4 border-t border-slate-100 dark:border-slate-700">
                   <div className="flex items-center justify-between">
                     <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('reminder_settings')}</span>
                     <ToggleSwitch checked={notifSettings.appointments} onChange={() => { triggerHaptic('light'); setNotifSettings(p => ({...p, appointments: !p.appointments})); }} />
                   </div>
                   <div className="flex items-center justify-between">
                     <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('blog_settings')}</span>
                     <ToggleSwitch checked={notifSettings.blog} onChange={() => { triggerHaptic('light'); setNotifSettings(p => ({...p, blog: !p.blog})); }} />
                   </div>
                   <div className="flex items-center justify-between">
                     <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('ai_settings')}</span>
                     <ToggleSwitch checked={notifSettings.ai} onChange={() => { triggerHaptic('light'); setNotifSettings(p => ({...p, ai: !p.ai})); }} />
                   </div>
                 </div>
               )}
            </div>
            ...
            <button 
              ...
              onClick={() => { triggerHaptic('light'); setShowFeedbackModal(true); }}
            >
               <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-teal-50 dark:bg-teal-900/30 text-teal-500">
                     <MessageSquare size={20} />
                  </div>
                  <span className="font-medium text-slate-700 dark:text-slate-200 text-sm">ارسال بازخورد (Feedback)</span>
               </div>
               {isRtl ? <ChevronLeft size={18} className="text-slate-300" /> : <ChevronRight size={18} className="text-slate-300" />}
            </button>

            <MenuRow icon={<Shield />} color="text-emerald-500" bg="bg-emerald-50 dark:bg-emerald-900/30" title={t('privacy_security')} isRtl={isRtl} />
            <MenuRow icon={<LogOut />} color="text-slate-400 dark:text-slate-500" bg="bg-slate-50 dark:bg-slate-800" title={t('logout')} last isRtl={isRtl} />
         </div>
      </div>
        ...
        <div className="absolute inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
           <div className="bg-white dark:bg-slate-800 w-full max-w-sm p-6 rounded-3xl shadow-xl">
              <div className="flex justify-between items-center mb-4">
                 <h3 className="font-bold text-slate-900 dark:text-white">ارسال بازخورد</h3>
                 <button onClick={() => setShowFeedbackModal(false)} className="text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 p-1.5 rounded-full">
                    <X size={20} />
                 </button>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">لطفا نظر، باگ یا پیشنهاد خود را وارد کنید.</p>
              ...
              <div className="flex justify-center gap-2 mb-4">
                 {[1,2,3,4,5].map(star => (
                   ...
                   >
                     <Star size={28} className={star <= rating ? 'fill-amber-400 text-amber-400' : 'text-slate-300 dark:text-slate-600'} />
                   </button>
                 ))}
              </div>
...
function MenuRow({ icon, color, bg, title, last = false, isRtl }: any) {
  return (
    <button className={`w-full flex items-center justify-between p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${!last ? 'border-b border-slate-50 dark:border-slate-700' : ''}`}>
       <div className="flex items-center gap-3">
